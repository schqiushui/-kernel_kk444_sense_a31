...
#if defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_A31_DTUL) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_A31_UL)
...
{
    if (!strcmp(str, "true")) {
        a31_dtul_battery_id = 2;
        pr_info("[BATT] battery detected current SKU is China OC or CU(Weike)\n");
    } else {
        a31_dtul_battery_id = 1;
        pr_info("[BATT] battery detected current OS is a31 other SKUs(Jiade)\n");
    }
    return 0; 
}

__setup("dtul_weike=", check_battery_setup);
#endif
